...
def generate_cohort_cuimc(config):
    ...
    # Get counts of labels in subjects
    ...
    #for j, files in enumerate([train_files]):
        for i, file in enumerate(files):
            ...
            #     .group_by(["code", "concept_name"])
            #     .agg(pl.len().alias("count"))
            #     .sort("count", descending=True)
            #     .head(100)
            # )
            # print(top_codes.select(["code", "concept_name", "count"]).to_pandas().to_string())

            # df_oxygen = df_measurements.filter(pl.col("code") == "LOINC/33037-3")
            # print(df_oxygen)
...
def generate_cohort_mimic(config):
    ...
    # admissions = pl.read_csv("/data/raw_data/mimic/files/mimiciv/3.1/hosp/admissions.csv.gz")
